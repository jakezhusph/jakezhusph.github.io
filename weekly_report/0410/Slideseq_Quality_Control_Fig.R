...
df$diff <- df$zeroPercent-df$expZ
...
df_nomt$diff <- df_nomt$zeroPercent-df_nomt$expZ
...
p7  <- ggplot() +
        geom_point(data=df, aes(x=log10(meanx), y=diff))+
        geom_abline(aes(intercept=0,slope=0,linetype="dashed"),color="green")+
        labs(title="With Mitochondrial Genes",
            x="Mean (log10)", y ="Difference (obs-exp)")+
        theme_bw()+
        theme(plot.title = element_text(hjust = 0.5,size=20),
                axis.text.y=element_text(size=15),
                axis.text.x=element_text(size=15),
...
p8  <- ggplot() +
        geom_point(data=df_nomt, aes(x=log10(meanx), y=diff))+
        geom_abline(aes(intercept=0,slope=0,linetype="dashed"),color="green")+
        labs(title="Without Mitochondrial Genes",
            x="Mean (log10)", y ="Difference (obs-exp)")+
        theme_bw()+
...
zero_mean_fig   <- ggarrange(p5,p6,p7,p8,ncol=2,nrow=2)
...
#+ label=fig2,fig.width=12, fig.height=12, echo=F,fig.align="center"
zero_mean_fig


#'***
#' **Gene-wise Dispersion, Genes expressed in less than three spots are removed**  
#'***

#' > Variance vs. Mean Figures
#+ label=plot2, echo=F, warnings=F, message=F,eval=T
rm(list=ls())
library(Rcpp)
library(fitdistrplus)
library(ggplot2)
...
# based on https://github.com/gokceneraslan/fit_nbinom
# cut at 1 {'size': 0.004632763012861035, 'prob': 0.36502048296596346}
# cut at 2 {'size': 0.0043396612354947945, 'prob': 0.36510410433544876}
r = 0.004632763012861035
p = 0.36502048296596346
phi1 = 1/r
# phi1 = coef(nls(var_gene ~ mean_gene + phi * mean_gene^2, start = list(phi = 0.1)))
...
# var-mean figures
# var_mean_fig  <- ggarrange(p1,p2,p3,p4,ncol=2,nrow=2)
var_mean_fig    <- ggarrange(p1,p2,ncol=2,nrow=1)
...
#'***
#' > Comparing observed with expected zeros 
#+ label=fig4,fig.width=12, fig.height=6, echo=F,fig.align="center"
zero_mean_fig
