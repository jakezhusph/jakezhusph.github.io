...
for(iblock in c(38,5,1)){
	...
	for(ilayer in 1:3){
		...
		for(isid in 1:10){
			allpval <- c()
			if(iblock!=1){
				load(paste0(outpath,"/sim_kdc_joint_CN24_D1_",iblock,"X_",layerlist[ilayer],"_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_rpt",isid,".rds"))
			}else{
	...
	save(kdc_power,file=paste0(respath,"/sim_kdc_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_power.rds"))
	print(kdc_power)
	cat("----------------------------------------------------------------\n")
    cat("----------------------------------------------------------------\n")
    cat("\n")
...
for(iblock in c(38)){
	...
	save(spark_power,file=paste0(respath,"/sim_spark_mixture_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_lmm_power.rds"))
	print(spark_power)
	cat("----------------------------------------------------------------\n")
    cat("----------------------------------------------------------------\n")
    cat("\n")
...
for(iblock in c(38)){
	...
	save(spe_power,file=paste0(respath,"/sim_spe_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_power.rds"))
	print(spe_power)
	cat("----------------------------------------------------------------\n")
	cat("----------------------------------------------------------------\n")
}
...
for(sigZero in c(100,200,500)){
	...
	for(iblock in c(38)){
		...
		load(paste0(respath,"/sim_spark_mixture_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_lmm_power.rds"))
		load(paste0(respath,"/sim_kdc_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_power.rds"))


		load(paste0(respath,"/sim_spe_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_power.rds"))
		...
		for(ilayer in 1:3){
			...
			if(ilayer==1){
				single_sample_power_fig[[ilayer]] <- powerplot_gg(x=seq(0,0.1,by=0.01),Power_dat=pltdat,linesize=2,ylim=c(0,1),xlim=c(0,0.105),legend.position=c(.7,0.5),
...
for(sigZero in c(100,200,500)){
	numZero = sigZero*10
	icount = icount + 1
	load(paste0(respath,"/sim_kdc_CN24_D1_38X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_power.rds"))
	x38 <- kdc_power
	rm(kdc_power)

	load(paste0(respath,"/sim_kdc_CN24_D1_5X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_power.rds"))
	x5 <- kdc_power
	rm(kdc_power)

	load(paste0(respath,"/sim_kdc_CN24_D1_1X_zero",numZero,"_sigzero",sigZero,"_means_blocked_binR_power.rds"))
	xhdst <- kdc_power
	rm(kdc_power)
