#' ---
#' title: "Power Comparison for Simulation Based on HDST (Average Count)"
#' author: "Jiaqiang Zhu"
#' date: "April 3rd, 2020"
#' ---
...
for(iblock in c(50,25,10,5,1)){
	if(iblock != 1){
		load(paste0("/net/mulan/disk2/jiaqiang/kdc/experiment/manuscript/simulation/v3/data/rds/sim_data_mean_CN24_D1_",iblock,"X_ONL_zero",numZero,"_sigzero",sigZero,"_rpt1.rds"))
	}else{
	...
	for(ilayer in 1:3){
		...
		for(isid in 1:10){
			allpval <- c()
			# if(iblock!=1){
			load(paste0(outpath,"/sim_kdc_joint_mean_CN24_D1_",iblock,"X_",layerlist[ilayer],"_zero",numZero,"_sigzero",sigZero,"_rpt",isid,".rds"))
	...
	save(kdc_power,file=paste0(respath,"/sim_kdc_mean_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	print(kdc_power)
	cat("----------------------------------------------------------------\n")
    cat("----------------------------------------------------------------\n")
    cat("\n")
...
for(iblock in c(50,25,10)){
	...
	for(ilayer in 1:3){
		single_power <- c()
		for(isid in 1:10){
			load(paste0(outpath,"/sim_spark_mean_mixture_CN24_D1_",iblock,"X_",layerlist[ilayer],"_zero",numZero,"_sigzero",sigZero,"_rpt",isid,".rds"))
			single_power <- cbind(single_power,sim_power_func(spark_pval$combined_pval,upfdr=0.1,numTrue=100,totalGene=1000)$power)
		}
		all_power[[ilayer]] <- single_power
	}

	spark_power <- setNames(data.frame(fdr=seq(0,0.1,by=0.01),sapply(all_power,function(x){apply(x,1,mean)})),c("FDR",layerlist))
	save(spark_power,file=paste0(respath,"/sim_spark_mean_mixture_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	print(spark_power)
	cat("----------------------------------------------------------------\n")
    cat("----------------------------------------------------------------\n")
    cat("\n")
...
#+ label=SPARK_gau, echo=F, warnings=F, message=F,eval=F
rm(list=setdiff(ls(),"sim_power_func"))
source("/net/mulan/disk2/jiaqiang/kdc/code/v5/kdc.R")
sim_power_func <- function(pval,upfdr=0.2,numTrue=1000,totalGene=10000){
        out         <- cbind(pval,c(rep(1,numTrue),rep(0,totalGene-numTrue)))
...
for(iblock in c(50,25,10)){
	...
	for(ilayer in 1:3){
		single_power <- c()
		for(isid in 1:10){
			load(paste0(outpath,"/sim_spark_mean_ten_gaussian_CN24_D1_",iblock,"X_",layerlist[ilayer],"_zero",numZero,"_sigzero",sigZero,"_rpt",isid,".rds"))
			single_power <- cbind(single_power,sim_power_func(spark_pval$combined_pval,upfdr=0.1,numTrue=100,totalGene=1000)$power)
		}
		all_power[[ilayer]] <- single_power
	}
	...
	spark_power <- setNames(data.frame(fdr=seq(0,0.1,by=0.01),sapply(all_power,function(x){apply(x,1,mean)})),c("FDR",layerlist))
	save(spark_power,file=paste0(respath,"/sim_spark_mean_ten_gaussian_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	print(spark_power)
	cat("----------------------------------------------------------------\n")
    cat("----------------------------------------------------------------\n")
    cat("\n")
...
for(iblock in c(50,25,10)){
	icount = icount + 1
	load(paste0(respath,"/sim_spark_mean_mixture_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	load(paste0(respath,"/sim_kdc_mean_CN24_D1_",iblock,"X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
...
#' ***
#' > Figure 3: 10X, numZero = 2000, sigZero = 200
...
#' **Power Figures II (Fig.4-6), numZero = 1000, sigZero = 100.**    
...
#' ***
#' > Figure 4: 50X, numZero = 1000, sigZero = 100

#+ label=fig4,fig.width=9, fig.height=3, echo=F,fig.align="center",eval=T
fig1


#' ***
#' > Figure 5: 25X, numZero = 1000, sigZero = 100

#+ label=fig5,fig.width=9, fig.height=3, echo=F,fig.align="center",eval=T
fig2



#' ***
#' > Figure 6: 10X, numZero = 1000, sigZero = 100
#+ label=fig6,fig.width=9, fig.height=3,  echo=F,fig.align="center",eval=T
fig3



#' **Power Figures III (Fig.7-Fig.9), numZero = 5000, sigZero = 500.**  
...
fig3 <- ggarrange(power_fig[[3]][[1]]+rremove("legend"), power_fig[[3]][[2]],power_fig[[3]][[3]],labels = c("A", "B","C"),font.label=list(size=12),ncol = 3, nrow = 1)

#' ***
#' > Figure 7: 50X, numZero = 5000, sigZero = 500

#+ label=fig7,fig.width=9, fig.height=3, echo=F,fig.align="center",eval=T
fig1


#' ***
#' > Figure 8: 25X, numZero = 5000, sigZero = 500

#+ label=fig8,fig.width=9, fig.height=3, echo=F,fig.align="center",eval=T
fig2



#' ***
#' > Figure 9: 10X, numZero = 5000, sigZero = 500

#+ label=fig9,fig.width=9, fig.height=3, echo=F,fig.align="center",eval=T
fig3
...
for(sigZero in c(100,200,500)){
	numZero = sigZero*10
	icount = icount + 1
	load(paste0(respath,"/sim_kdc_mean_CN24_D1_50X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	x50 <- kdc_power
	rm(kdc_power)

	load(paste0(respath,"/sim_kdc_mean_CN24_D1_25X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	x25 <- kdc_power
	rm(kdc_power)

	load(paste0(respath,"/sim_kdc_mean_CN24_D1_10X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	x10 <- kdc_power
	rm(kdc_power)

	load(paste0(respath,"/sim_kdc_mean_CN24_D1_5X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	x5 <- kdc_power
	rm(kdc_power)

	load(paste0(respath,"/sim_kdc_mean_CN24_D1_1X_zero",numZero,"_sigzero",sigZero,"_power.rds"))
	xhdst <- kdc_power
	rm(kdc_power)
	...
	power_fig[[icount]] <- single_sample_power_fig
	rm(single_sample_power_fig,x50,x25,x10,x5,xhdst)
}
...
#' ***
#' > Figure 10: numZero = 5000, sigZero = 500

#+ label=fig10,fig.width=9, fig.height=3, echo=F,fig.align="center"
fig3



#' ***
#' > Figure 11: numZero = 2000, sigZero = 200

#+ label=fig11,fig.width=9, fig.height=3,echo=F,fig.align="center"
fig2


#' ***
#' > Figure 12: numZero = 1000, sigZero = 100

#+ label=fig12,fig.width=9, fig.height=3, echo=F,fig.align="center"
fig1
