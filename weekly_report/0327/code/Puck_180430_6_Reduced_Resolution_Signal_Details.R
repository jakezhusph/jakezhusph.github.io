#' ---
#' title: "Slideseq Reduced Resolution Signal Comparison Details"
#' author: "Jiaqiang Zhu"
...
#' ***
#' > Figure 0: Venn Across Methods at Different Resoultions: 50, 25, 20,
#+ label=venn1,fig.width=12, fig.height=4, fig.cap="Venn Diagrams",echo=F,fig.align="center"
fig0
...
names(signal_list) <- paste0("X",c(200,100,50))
...
#+ echo=F, warnings=F, message=F,eval=T
allgene <- unique(unlist(signal_list))
pd <- cbind.data.frame(X200=allgene%in%signal_list[[1]],
						X100=allgene%in%signal_list[[2]],
						X50=allgene%in%signal_list[[3]])
...
names(signal_list) <- paste0("X",c(200,100,50,1))

allgene <- unique(unlist(signal_list))
pd <- cbind.data.frame(X200=allgene%in%signal_list[[1]],
						X100=allgene%in%signal_list[[2]],
						X50=allgene%in%signal_list[[3]],
						X1=allgene%in%signal_list[[4]])
rownames(pd) <- allgene
library(eulerr)
VennDiag <- euler(pd,shape="ellipse") ## other wise some number if missing 
library(ggsci)
...
# venn
venn_list <- list()
sq_color 	<- c("hotpink","lightskyblue")
icount = 0
for(iblock in c(200,100,50)){
	icount <- icount+1
	plt_list 	<- list(KDC=kdc_gene,SPARK=signal_list[[paste0("X",iblock)]])
	allgene 	<- unique(unlist(plt_list))

	pd <- cbind.data.frame(KDC=allgene%in%plt_list[[1]],
							SPARK=allgene%in%plt_list[[2]])
	rownames(pd) <- allgene
	library(eulerr)
	library(ggsci)
	...
	VennDiag 	<- euler(pd,shape="ellipse") ## other wise some number if missing 
	if(icount==3){
		venn_list[[icount]] 	<- plot(VennDiag, quantities = T, lty=rep(0,ncol(pd)),fill=sq_color,alpha=1,legend=T,labels=F)
	}else{
		venn_list[[icount]] 	<- plot(VennDiag, quantities = T, lty=rep(0,ncol(pd)),fill=sq_color,alpha=1,legend=F,labels=F)
	}	
}

library(ggpubr)
venn2 <- ggarrange(venn_list[[1]], venn_list[[2]], venn_list[[3]],
			labels = c("A", "B", "C"),
			font.label=list(size=20),
			ncol = 3)
...
#' ***
#' > Figure 3: Venn KDC (1X) vs. SPARK(200X,100X,50X)
#+ label=venn2,fig.width=12, fig.height=4, fig.cap="Venn Diagrams",echo=F,fig.align="center"
venn2
