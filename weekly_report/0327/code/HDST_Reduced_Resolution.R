...
for(iblock in c(50,25,20,15,10)){
	load(paste0(workdir,"/data/clean_",isample,"_idx_",iblock,"X_sparseCount.rds"))
...
for(iblock in c(50,25,20,15,10)){
	...
	spark <- CreateSPARKObject(counts = sp_count, location = raw_loc, percentage = 0, min_total_counts = 10)
	spark@lib_size <- apply(spark@counts, 2, sum)
	# spark@counts <- spark@counts[1:10,]
	t1 		<- proc.time()
	spark 	<- spark.vc(spark, covariates = NULL, lib_size = spark@lib_size, 
...
isample = "CN24_D1"
for(iblock in c(50,25,20,15,10)){
	res <- read.table(paste0(workdir,"/output/spatialDE/clean_CN24_D1_idx_",iblock,"X_spe.csv"),head=T)
	cat("------------------------------------\n")
...
isample = "CN24_D1"
for(iblock in c(50,25,20,15,10)){
	res <- read.table(paste0(workdir,"/output/spatialDE/clean_CN24_D1_idx_",iblock,"X_spe_nomt.csv"),head=T)
	cat("------------------------------------\n")
	cat(iblock,"X: sampleSize",res$n[1],"numGene",nrow(res)-1,"\n")
	cat("signal genes:",sum(res$qval<0.05),"\n")
	rm(res)
...
for(iblock in c(50,25,20,15,10,5,1)){
	kdc_pval <- c()
	for(isid in 1:10){
		load(paste0(kdcpath,"/clean_CN24_D1_idx_",iblock,"X_Joint_kdc_perm",isid,".rds"))
		kdc_pval 	<- cbind(kdc_pval,KDC$res_mtest$combinedPval)
		gn 			<- rownames(KDC$res_mtest)
		rm(KDC)
	}
	rownames(kdc_pval) <- gn
	save(kdc_pval,file=paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_KDC_permutation_pval.rds"))
	rm(kdc_pval)
}
...
for(iblock in c(50,25,20,15,10)){
	spe_pval <- c()
	for(isid in 1:10){
		spe 	<- read.table(paste0(spepath,"/clean_CN24_D1_idx_",iblock,"X_spe_perm",isid,"_nomt.csv"),head=T)
	...
	save(spe_pval,file=paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_spe_permutation_pval_nomt.rds"))
	rm(spe_pval)
}
...
for(iblock in c(50,25,20,15,10)){
	spark_pval <- c()
	for(isid in 1:10){
		load(paste0(sparkpath,"/clean_CN24_D1_idx_",iblock,"X_spark_default_nomt_perm",isid,".rds"))
		spark_pval 	<- cbind(spark_pval,spark@res_mtest$combined_pvalue)
		rm(spark)
	}
	save(spark_pval,file=paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_spark_permutation_pval_nomt.rds"))
}
...
for(iblock in c(50,25,20,15,10)){
	...
	save(spark_pval,file=paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_spark_permutation_pval_nomt_converged.rds"))
}
...
icount = 0
for(iblock in c(50,25,20,15,10,5,1)){
	load(paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_KDC_permutation_pval.rds"))
	icount = icount + 1
	permlist[[icount]] <- kdc_pval
	rm(kdc_pval)
}

names(permlist) <- paste0("X",c(50,25,20,15,10,5,1))
KDC_qq 			<- qplot_gg(permlist,cl=0,legend.position=c(0.15,0.8),pt.size=2,
					        ax.txt.size=10,ax.title.size=10,len.txt.size=1,
					        factor_level=paste0("X",c(50,25,20,15,10,5,1)),
					        self_label=paste0("X",c(50,25,20,15,10,5,1)))
...
for(iblock in c(50,25,20,15,10)){
	icount = icount + 1
	load(paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_spe_permutation_pval_nomt.rds"))
	permlist[[icount]] <- spe_pval
	rm(spe_pval)
}
names(permlist) <- paste0("X",c(50,25,20,15,10))
spe_qq 			<- qplot_gg(permlist,cl=0,legend.position=c(0.15,0.8),pt.size=2,
					        ax.txt.size=10,ax.title.size=10,len.txt.size=1,
					        factor_level=paste0("X",c(50,25,20,15,10)),
					        self_label=paste0("X",c(50,25,20,15,10)))
...
for(iblock in c(50,25,20,15,10)){
	icount = icount + 1
	load(paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_spark_permutation_pval_nomt.rds"))
...
for(iblock in c(50,25,20,15,10)){
	icount = icount + 1
	load(paste0(respath,"/clean_CN24_D1_idx_",iblock,"X_spark_permutation_pval_nomt_converged.rds"))
	permlist[[icount]] <- spark_pval
	rm(spark_pval)
}
names(permlist) <- paste0("X",c(50,25,20,15,10))
