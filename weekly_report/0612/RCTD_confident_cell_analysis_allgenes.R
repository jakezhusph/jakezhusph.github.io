#' ---
#' title: "RCTD Confident Cell Analysis - All Genes"
#' author: "Jiaqiang Zhu"
#' date: "June 12th, 2020"
#' ---


#' **After removing genes with zero-expression in all cells or genes with NA values (due to the doublet split). A final dataset of 7177 cells and 7482 genes is analyzed **  
...
res1 <- res2 <- c()
for(iper in 1:10){
    load(paste0(workdir,"/kdc/kdc_RCTD_modified_allgenes_decomposed_per",iper,"_XTX.rds"))
...
res3 <- c()
for(iper in 1:10){
    load(paste0(workdir,"/kdc/kdc_RCTD_modified_allgenes_decomposed_per",iper,".rds"))
...
load(paste0(workdir,"/kdc/fdr_kdc_RCTD_modified_allgenes_decomposed.rds"))
...
load(paste0(workdir,"/kdc/fdr_kdc_RCTD_modified_allgenes_decomposed_XTX.rds"))
...
for(iter in c(".",".XTX.")){
    load(paste0(workdir,"/enrichment/res.RCTD.modified.allgenes.enrichGO.diff2all.kdc",iter,"rds"))

    load(paste0(workdir,"/enrichment/res.RCTD.modified.allgenes.enrichKEGG.diff2all.kdc",iter,"rds"))
